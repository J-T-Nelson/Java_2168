...
/**
 * This file includes code in prerecorded videos and Lec 5.
 * Some methods includes multiple solutions.
 * This file does not include answers to the linked list in-class practice.
 * You can find them in other packages.
 *
 * Class to represent a single linked list, with a link from each node to the
 * next node.
 * A Generic class, representing a single linked list of objects of any non-primitive type.
 * SingleLinkedList class (the book version) does not implement the java.util.List interface.
 * Instead, the class implements only the following public methods in List<E> interface:
 *     get(int index), set(int index, E data),
 *     add(E data), add(int index, E data),
 *     toString()
 *
 * @author Koffman and Wolfgang and Cindy
 */
//mark the class as generic using <E>. E is the type parameter for list elements.
public class SingleLinkedList<E> {
    ...
    public static void main(String[] args) {
        //create 3 standalone nodes using Node<E> constructor: Node(E data)
        Node<Integer> first = new Node<>(20);
        Node<Integer> second = new Node<>(60);
        Node<Integer> third = new Node<>(100);

        //use Node<E> toString() to print node data
        System.out.println(first.toString());
        System.out.println(first);

        //use constructor: Node<E data, Node<E> next)
        //create a node with data 200 and successor: node for 100.
        Node<Integer> newNode = new Node<>(200, third);
        //print the data in the new node's successor node
        System.out.println(newNode.next.toString());
        //The line below is equivalent to the line above
        System.out.println(newNode.next);

        //directly connect nodes to form a list: 20, 60, 100
        first.next = second;
        second.next = third;

        Node<Integer> last = third;
        //The line below is the same as System.out.println(first.next.toString());
        //  It prints the second item in the list.
        System.out.println(first.next);
        //The line below is the same as System.out.println(last.toString());
        ...
        Node<String> firstNode = tom;
        Node<String> lastNode = sam;
        System.out.println(firstNode.data);         //print first name
        System.out.println(firstNode.next.data);    //print second name
        //print the third name: Harry using firstNode
     ...
     * This method can also be private.
    ...
    private E removeAfter(Node<E> node) { //Given node: the node referenced by the parameter: node
        ...
        if (node.next != null) {            //if the sucessor exists
            E deletedItem = node.next.data; //save deleted item
     ...
     *        Precondition: index is valid before entering the method.
     * @returns The node at index or null if it does not exist
     */
    private Node<E> getNode(int index) {
        Node<E> current = head;                //make the first node the current node to check
        ...
        while (i < index) {             //not reached the target index and not checked all nodes yet
     ...
     * Store a reference to the new item at the given position (index)
     *
     * @param index The position of the item to be changed
    ...
    public E set(int index, E newItem) {
        ...
        Node<E> targetNode = getNode(index);  //call private method getNode(..) to get the desired node
        E oldItem = targetNode.data;
        targetNode.data = newItem;
        return oldItem;
    }
    ...
    public boolean add(E item) {
        ...
        //Solution 2: code without calling other methods
        /*
        Node<E> newNode = new Node<>(item);

        //empty list
        if (head == null) {
            head = newNode;     //change data field head
        } else {//non-empty list
            //locate last node, starting from the first node
            Node<E> current = head;
            while (current.next != null) {
                current = current.next;
            }
            //make newNode the successor of the last node
